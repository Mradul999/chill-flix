...
export const GPTsearch = () => {
  ...
  return (
    <div className="min-h-screen w-screen background    ">
      <div className=" flex justify-center pt-28 sm:pt-16 pb-10 ">
        <form onSubmit={submitHandler} className="flex flex-col gap-4 mx-3   ">
          <input
            value={query}
            onChange={onchangeHandler}
            placeholder="Search by movie title"
            className="focus:outline-none border-white border-[1.3px] rounded-md py-2  sm:w-[400px] w-[300px] lg:w-[500px] xl:w-[600px] text-[15px] text-white    bg-black pl-6  bg-opacity-0"
            type="text"
          ></input>
          <select
            value={category}
            onChange={CategoryHandler}
            className="bg-black text-gray-300 placeholder:text-white border-white border-[1px] py-2 focus:outline-none pl-6 text-[15px] rounded-md bg-opacity-0"
          >
